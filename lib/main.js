...
var runiform = require( '@stdlib/random-base-uniform' ).factory;
...
function RandomStream( a, b, options ) {
	...
	if ( !isNumber( a ) || isnan( a ) ) {
		throw new TypeError( format( 'invalid argument. First argument must be a number primitive and not `NaN`. Value: `%s`.', a ) );
	}
	if ( !isNumber( b ) || isnan( b ) ) {
		throw new TypeError( format( 'invalid argument. Second argument must be a number primitive and not `NaN`. Value: `%s`.', b ) );
	}
	if ( a >= b ) {
